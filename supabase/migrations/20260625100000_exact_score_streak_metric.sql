-- Add current_exact_score_streak to compute_event_tag_metrics
-- Mirrors ordered_winner/streak_correct pattern for exact_score predictions.
-- Used by The Professor tag (2+ consecutive exact scores).
...
RETURNS TABLE (
  ...
  current_exact_score_streak     integer
)
LANGUAGE sql
SECURITY DEFINER
STABLE
...
-- Exact score streak (consecutive exact scores ending at current event)
ordered_exact AS (
  SELECT
    p.user_id,
    COALESCE(p.is_correct, false) AS is_correct,
    ROW_NUMBER() OVER (
      PARTITION BY p.user_id
      ORDER BY ace.pos DESC
    ) AS rev
...
SELECT
  ...
  COALESCE(se.streak, 0)                        AS current_exact_score_streak
FROM predictors pr
...
LEFT JOIN streak_exact se    ON se.user_id = pr.user_id
...
COMMENT ON FUNCTION public.compute_event_tag_metrics IS
  ...
  'streaks (winner + exact score), leaderboard position delta, and timing. '
