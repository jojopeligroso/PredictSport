...
export async function GET(request: NextRequest) {
  ...
  // Per-user aggregation: winner accuracy + total points
  const userStats = new Map<string, { correct: number; total: number; points: number }>();
  for (const p of predictions) {
    const stats = userStats.get(p.user_id) ?? { correct: 0, total: 0, points: 0 };
  ...
  // Current user's accuracy
  const self = userStats.get(user.id);
  const userAccuracy = self && self.total > 0
    ? {
        correct: self.correct,
        total: self.total,
        pct: Math.round((self.correct / self.total) * 100),
      }
    : null;
