...
- [ ] **SH-4 — Multi-provider cross-validation for auto-resolved results** — After primary provider (ESPN) returns a final result, cross-check score with a second provider (TheSportsDB or API-Football) before scoring predictions. If scores agree, proceed. If they disagree, try a third provider as tiebreaker. If no consensus within 180s, hold for admin review. Design decisions needed: (1) timing — score immediately + async verify, or hold until verified; (2) event ID reconciliation across providers (ESPN ID ≠ API-Football ID); (3) compare only home_score/away_score (two integers); (4) rate budget (API-Football = 100 req/day); (5) disagreement handling (hold vs auto-correct); (6) scope — soccer-only initially (4 providers) or generic; (7) wrap existing fetchResult() or replace. Run `/grill-with-docs` before implementation. *(Added 2026-06-20 after ESPN "United States" vs "USA" scoring incident — positional derivation fix deployed but single-provider trust model remains.)*
